// Package getopt provides a zero-allocation POSIX flag parser.
//
// It supports clustered short flags (-laR), long flags (--name),
// --name=value, --name value, -- end-of-flags, bare - stdin marker,
// optional-value flags (-e[eof-str]), and flag repetition counting (-vvv).
package getopt
...
// Public fields preserved for backward compatibility with direct map access.
...
func (cs *compiledSpec) parseLong(args []string, i int, res *ParseResult) (int, error) {
	...
	// Scan for '=' inline (no strings.IndexByte).
...
func (cs *compiledSpec) parseShort(args []string, i int, res *ParseResult) (int, error) {
	arg := args[i]
	// arg is "-..." with len >= 2.
	chars := arg[1:] // the flag characters after '-'
	...
	for ci := 0; ci < len(chars); ci++ {
		...
		// Value or OptionalValue flag — consumes the rest of the cluster.
		...
		// No remainder — value is next arg, or optional-empty.
		if cf.Type == FlagOptionalValue {
			if cf.Short != "" {
				res.Values[cf.Short] = ""
		...
		// FlagValue: consume next argument.
		if i+1 >= len(args) {
