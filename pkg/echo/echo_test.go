...
func TestRunCLIBasic(t *testing.T) {
	...
	code := run([]string{"hello", "world"}, nil, &buf, &buf, "")
	if code != 0 {
...
func TestRunCLINoNewline(t *testing.T) {
	var buf bytes.Buffer
	code := run([]string{"-n", "hello"}, nil, &buf, &buf, "")
	if code != 0 {
		t.Fatalf("exit code %d, want 0", code)
	}
	if buf.String() != "hello" {
...
func TestRunCLIJSON(t *testing.T) {
	var buf bytes.Buffer
	code := run([]string{"--json", "hello", "world"}, nil, &buf, &buf, "")
	if code != 0 {
		t.Fatalf("exit code %d, want 0", code)
	}
...
func TestRunCLIJSONNoNewline(t *testing.T) {
	var buf bytes.Buffer
	code := run([]string{"--json", "-n", "hello"}, nil, &buf, &buf, "")
	if code != 0 {
		t.Fatalf("exit code %d, want 0", code)
	}
	var env map[string]interface{}
...
func TestBusyBox_Echo_NonOptsFlagGroup(t *testing.T) {
	...
	var buf bytes.Buffer
	code := run([]string{"-neEZ"}, nil, &buf, &buf, "")
	if code != 0 {
		t.Fatalf("exit code %d, want 0", code)
	}
	if buf.String() != "-neEZ\n" {
...
func TestBusyBox_Echo_OctalEscapeNNN(t *testing.T) {
	...
	var buf bytes.Buffer
	code := run([]string{"-ne", `\41z`}, nil, &buf, &buf, "")
	if code != 0 {
		t.Fatalf("exit code %d, want 0", code)
	}
	if buf.String() != "!z" {
...
func TestBusyBox_Echo_OctalEscapeZeroNNN(t *testing.T) {
	...
	var buf bytes.Buffer
	code := run([]string{"-ne", `\041`}, nil, &buf, &buf, "")
	if code != 0 {
		t.Fatalf("exit code %d, want 0", code)
	}
	if buf.String() != "!" {
