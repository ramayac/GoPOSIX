...
func TestRunTestCLI(t *testing.T) {
	...
	code := runTest([]string{"hello", "=", "hello"}, nil, &buf, &buf, "")
	if code != 0 {
...
func TestRunTestCLIFalse(t *testing.T) {
	var buf bytes.Buffer
	code := runTest([]string{"hello", "=", "world"}, nil, &buf, &buf, "")
	if code != 1 {
...
func TestRunTestCLIJSON(t *testing.T) {
	var buf bytes.Buffer
	code := runTest([]string{"--json", "hello", "=", "hello"}, nil, &buf, &buf, "")
	if code != 0 {
...
func TestRunTestJSONMiddleware(t *testing.T) {
	...
	var buf bytes.Buffer
	code := runTest([]string{"hello", "--json", "=", "hello"}, nil, &buf, &buf, "")
	if code != 0 {
...
func TestRunBracket(t *testing.T) {
	var buf bytes.Buffer
	code := runBracket([]string{"hello", "=", "hello", "]"}, nil, &buf, &buf, "")
	if code != 0 {
		t.Errorf("exit code %d, want 0", code)
	}
}
...
func TestRunBracketJSON(t *testing.T) {
	var buf bytes.Buffer
	code := runBracket([]string{"--json", "hello", "=", "hello", "]"}, nil, &buf, &buf, "")
	if code != 0 {
		t.Fatalf("exit code %d, want 0", code)
	}
	var env map[string]interface{}
