...
func TestCLI_ShowAll(t *testing.T) {
	...
	var out bytes.Buffer
	code := run([]string{"-a", dir}, nil, &out, &out, "")
	if code != 0 {
		t.Fatalf("exit code %d", code)
	}
}
...
func TestCLI_LongFormat(t *testing.T) {
	...
	var out bytes.Buffer
	code := run([]string{"-l", dir}, nil, &out, &out, "")
	if code != 0 {
		t.Fatalf("exit code %d", code)
	}
}
...
func TestCLI_OnePerLine(t *testing.T) {
	...
	var out bytes.Buffer
	code := run([]string{"-1", dir}, nil, &out, &out, "")
	if code != 0 {
		t.Fatalf("exit code %d", code)
	}
}
...
func TestCLI_Recursive(t *testing.T) {
	...
	var out bytes.Buffer
	code := run([]string{"-R", dir}, nil, &out, &out, "")
	if code != 0 {
		t.Fatalf("exit code %d", code)
	}
}
...
func TestCLI_JSON(t *testing.T) {
	...
	var out bytes.Buffer
	code := run([]string{"--json", dir}, nil, &out, &out, "")
	if code != 0 {
		t.Fatalf("exit code %d", code)
	}
...
func TestCLI_LongFlags(t *testing.T) {
	dir := t.TempDir()
	var out bytes.Buffer
	code := run([]string{"--all", dir}, nil, &out, &out, "")
	if code != 0 {
		t.Fatalf("exit code %d", code)
	}
}
...
func TestCLI_NotExist(t *testing.T) {
	var out bytes.Buffer
	code := run([]string{"/nonexistent/ls/path"}, nil, &out, &out, "")
	if code != 2 {
...
func TestCLI_MultipleDirs(t *testing.T) {
	...
	d2 := t.TempDir()
	var out bytes.Buffer
	code := run([]string{d1, d2}, nil, &out, &out, "")
	if code != 0 {
		t.Fatalf("exit code %d", code)
	}
}
...
func TestCLI_DirectoryMode(t *testing.T) {
	...
	var out bytes.Buffer
	code := run([]string{"-d", dir}, nil, &out, &out, "")
	if code != 0 {
		t.Fatalf("exit code %d", code)
	}
