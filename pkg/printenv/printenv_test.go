...
func TestCLI_SingleVar(t *testing.T) { os.Setenv("KOREGO_TEST_VAR", "hello"); var out bytes.Buffer; code := run([]string{"KOREGO_TEST_VAR"}, &out); if code != 0 { t.Fatalf("exit %d", code) }; if !strings.Contains(out.String(), "hello") { t.Errorf("expected 'hello', got: %s", out.String()) } }
...
func TestCLI_JSON(t *testing.T) { os.Setenv("JSON_VAR", "val"); var out bytes.Buffer; code := run([]string{"-j", "JSON_VAR"}, &out); if code != 0 { t.Fatalf("exit %d", code) }; if !strings.Contains(out.String(), "\"vars\"") { t.Errorf("no JSON: %s", out.String()) } }
