...
func TestCLI_JSON(t *testing.T) { d := filepath.Join(t.TempDir(), "jd"); os.Mkdir(d, 0755); var out bytes.Buffer; code := run([]string{"-j", d}, &out); if code != 0 { t.Fatalf("exit %d", code) }; if !strings.Contains(out.String(), "\"removed\"") { t.Errorf("no JSON: %s", out.String()) } }
