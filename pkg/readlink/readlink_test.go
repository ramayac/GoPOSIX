...
func TestCLI_JSON(t *testing.T) { dir := t.TempDir(); target := filepath.Join(dir, "t"); os.WriteFile(target, []byte("x"), 0644); link := filepath.Join(dir, "l"); os.Symlink(target, link); var out bytes.Buffer; code := run([]string{"-j", link}, &out); if code != 0 { t.Fatalf("exit %d", code) }; if !strings.Contains(out.String(), "\"target\"") { t.Errorf("no JSON: %s", out.String()) } }
