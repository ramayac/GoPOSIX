...
func TestCLI_JSON(t *testing.T) { var out bytes.Buffer; code := run([]string{"-j"}, &out); if code != 0 { t.Fatalf("exit %d", code) }; if !strings.Contains(out.String(), "\"path\"") { t.Errorf("no JSON: %s", out.String()) } }
