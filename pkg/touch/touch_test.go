...
func TestCLI_ReferenceFile(t *testing.T) {
	...
	var out bytes.Buffer
	code := run([]string{"-r", ref, target}, nil, &out, &out, "")
	if code != 0 {
		t.Fatalf("exit code %d", code)
	}
...
func TestCLI_DateString(t *testing.T) {
	...
	var out bytes.Buffer
	code := run([]string{"-d", "2020-06-15 12:00:00", f}, nil, &out, &out, "")
	if code != 0 {
		t.Fatalf("exit code %d", code)
	}
	info, _ := os.Stat(f)
...
func TestCLI_TimeStamp(t *testing.T) {
	...
	var out bytes.Buffer
	code := run([]string{"-t", "202006151200.00", f}, nil, &out, &out, "")
	if code != 0 {
		t.Fatalf("exit code %d", code)
	}
	info, _ := os.Stat(f)
...
func TestCLI_BadReference(t *testing.T) {
	var out bytes.Buffer
	code := run([]string{"-r", "/nonexistent/ref", "/tmp/x"}, nil, &out, &out, "")
	if code != 1 {
...
func TestCLI_BadDate(t *testing.T) {
	...
	var out bytes.Buffer
	code := run([]string{"-d", "not-a-date", f}, nil, &out, &out, "")
	if code != 1 {
...
func TestCLI_LongFlags(t *testing.T) {
	...
	var out bytes.Buffer
	code := run([]string{"--no-create", f}, nil, &out, &out, "")
	if code != 0 {
		t.Fatalf("exit code %d", code)
	}
	if _, err := os.Stat(f); !os.IsNotExist(err) {
