...
func TestLinkCreatesHardLink(t *testing.T) {
	...
	code := run([]string{src, dst}, io.Discard)
	if code != 0 {
...
func TestLinkNonexistentSource(t *testing.T) {
	dir := t.TempDir()
	code := run([]string{filepath.Join(dir, "nonexistent"), filepath.Join(dir, "dst")}, io.Discard)
	if code != 1 {
...
func TestLinkJson(t *testing.T) {
	...
	code := run([]string{"--json", src, dst}, &buf)
	if code != 0 {
		t.Fatalf("expected exit 0, got %d", code)
	}
