...
func TestCLI_NumLines(t *testing.T) {
	f := tailTempFile(t, "1\n2\n3\n4\n5\n")
	var out bytes.Buffer
	code := run([]string{"-n", "2", f}, nil, &out, &out, "")
	if code != 0 {
		t.Fatalf("exit code %d", code)
	}
	lines := strings.Split(strings.TrimRight(out.String(), "\n"), "\n")
...
func TestCLI_TraditionalDashNum(t *testing.T) {
	f := tailTempFile(t, "1\n2\n3\n4\n5\n")
	var out bytes.Buffer
	code := run([]string{"-2", f}, nil, &out, &out, "")
	if code != 0 {
		t.Fatalf("exit code %d", code)
	}
	lines := strings.Split(strings.TrimRight(out.String(), "\n"), "\n")
...
func TestCLI_PlusNum(t *testing.T) {
	f := tailTempFile(t, "1\n2\n3\n4\n5\n")
	var out bytes.Buffer
	code := run([]string{"-n", "+3", f}, nil, &out, &out, "")
	if code != 0 {
		t.Fatalf("exit code %d", code)
	}
	lines := strings.Split(strings.TrimRight(out.String(), "\n"), "\n")
...
func TestCLI_BytesCount(t *testing.T) {
	...
	var out bytes.Buffer
	code := run([]string{"-c", "4", f}, nil, &out, &out, "")
	if code != 0 {
		t.Fatalf("exit code %d", code)
	}
	if out.String() != "ghij" {
...
func TestCLI_PlusBytesCount(t *testing.T) {
	f := tailTempFile(t, "abcdefghij")
	var out bytes.Buffer
	code := run([]string{"-c", "+5", f}, nil, &out, &out, "")
	if code != 0 {
		t.Fatalf("exit code %d", code)
	}
...
func TestCLI_LongFlags(t *testing.T) {
	f := tailTempFile(t, "1\n2\n3\n4\n5\n")
	var out bytes.Buffer
	code := run([]string{"--lines", "2", f}, nil, &out, &out, "")
	if code != 0 {
		t.Fatalf("exit code %d", code)
	}
	lines := strings.Split(strings.TrimRight(out.String(), "\n"), "\n")
...
func TestCLI_FileNotFound(t *testing.T) {
	var out bytes.Buffer
	code := run([]string{"/nonexistent/tail/file"}, nil, &out, &out, "")
	if code != 1 {
...
func TestCLI_IllegalLineCount(t *testing.T) {
	var out bytes.Buffer
	code := run([]string{"-n", "abc", "/dev/null"}, nil, &out, &out, "")
	if code != 2 {
...
func TestCLI_IllegalByteCount(t *testing.T) {
	var out bytes.Buffer
	code := run([]string{"-c", "xyz", "/dev/null"}, nil, &out, &out, "")
	if code != 2 {
...
func TestCLI_ShortFile(t *testing.T) {
	...
	var out bytes.Buffer
	code := run([]string{"-n", "10", f}, nil, &out, &out, "")
	if code != 0 {
		t.Fatalf("exit code %d", code)
	}
	lines := strings.Split(strings.TrimRight(out.String(), "\n"), "\n")
