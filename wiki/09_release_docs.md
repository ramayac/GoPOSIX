...
- [x] SBOM attached to archives and binaries (SPDX/cyclonedx, generated by GoReleaser)
- [x] Cosign keyless image signing (OIDC) in release workflow
- [x] SLSA Level 3 provenance attestation via `slsa-framework/slsa-github-generator`
- [x] Trivy vulnerability scanning on CI (CRITICAL/HIGH severity fail the build)

### 09.2b — Supply Chain Security (Phase 12.1)

Added post-Phase 09 as part of the Gold roadmap:

- **SBOM:** GoReleaser generates SPDX/cyclonedx SBOMs for both the binary and the
  archive. Consumers can inspect with `docker buildx imagetools inspect`.
- **Cosign:** The release workflow signs container images using keyless signing via
  OIDC (GitHub Actions identity). No long-lived keys to manage or rotate.
- **SLSA Level 3:** A separate `provenance` job in `release.yml` runs
  `slsa-framework/slsa-github-generator` to attest build provenance. The attestation
  includes commit SHA, workflow ref, and builder identity.
- **Trivy:** CI scans the Docker image on every push for CRITICAL and HIGH CVEs.
  The build fails if any are found.

See [docs/SECURITY.md](../docs/SECURITY.md) for verification instructions.
...
- [x] SBOM generated for all release artifacts
- [x] Cosign keyless signing of container images
- [x] SLSA Level 3 provenance attestations
...
- [x] **Supply chain security** (SBOM, Cosign, SLSA Level 3, Trivy scanning)
